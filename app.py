...
class NfeSystem(ctk.CTk):
    ...
    def login(self):
        ...
        try:
            ...
            self.show_toast("Erro ao conectar no banco.", "erro")
    ...
    def carregar_notas_cadastradas(self):
        ...
        try:
            ...
            query_total = "SELECT * FROM notas WHERE 1=1"
            params = []
            if self.f_txt.get(): 
                query_total += " AND (estabelecimento LIKE ? OR descricao LIKE ? OR categoria LIKE ?)"
                params.extend(['%'+self.f_txt.get()+'%', '%'+self.f_txt.get()+'%', '%'+self.f_txt.get()+'%'])
            if self.f_ini.get():
                dt = data_para_db(self.f_ini.get()); 
                if dt: query_total += " AND data >= ?"; params.append(dt)
            if self.f_fim.get():
                dt = data_para_db(self.f_fim.get()); 
                if dt: query_total += " AND data <= ?"; params.append(dt)
            
            cursor.execute(query_total, params)
            registros_totais = cursor.fetchall()

            resumo = calcular_resumo_financeiro(registros_totais)
            ...
            query_grupo = "SELECT data, categoria, tipo, SUM(valor), COUNT(id) FROM notas WHERE 1=1"
            if self.f_txt.get(): query_grupo += " AND (estabelecimento LIKE ? OR descricao LIKE ? OR categoria LIKE ?)"
            if self.f_ini.get():
                dt = data_para_db(self.f_ini.get())
                if dt: query_grupo += " AND data >= ?"
            if self.f_fim.get():
                dt = data_para_db(self.f_fim.get())
                if dt: query_grupo += " AND data <= ?"

            query_grupo += " GROUP BY data, categoria, tipo ORDER BY data DESC"
            cursor.execute(query_grupo, params)
